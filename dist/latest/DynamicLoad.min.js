...
1;0<=b;b--)if(-1!=e&&this.cloneNodes[e].data===d[b])e--;else{for(var g=this.bindedNodes(d[b]),f=-1==e?this.element?this.element.nextSibling:this.parent.firstChild:this.cloneNodes[e].nodes[this.cloneNodes[e].nodes.length-1].nextSibling,h=g.length;0<=h;h--)this.parent.insertBefore(g[h],f);this.cloneNodes.splice(e+1,0,{data:d[b],cloneData:JSON.parse(JSON.stringify(d[b])),nodes:g})}return this};a.prototype.bindedNodes=function(a){var c=document.createElement("div");c.innerHTML=this.elemHTML;if(a)for(var b=
Object.keys(a),e=0;e<b.length;e++)c.innerHTML=c.innerHTML.replace(new RegExp("{{"+b[e]+"}}","g"),a[b[e]]);return c.childNodes};return a}();f.Element=k})(DynamicLoad||(DynamicLoad={}));
