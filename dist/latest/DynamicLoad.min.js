...
for(var g=this.cloneNodes.length-1,b=d.length-1;0<=b;b--)if(-1!=g&&this.cloneNodes[g].data===d[b])g--;else{for(var f=this.bindedNodes(d[b]),k=-1==g?this.element?this.element.nextSibling:this.parent.firstChild:this.cloneNodes[g].nodes[this.cloneNodes[g].nodes.length-1].nextSibling,e=f.length-1;0<=e;e--)this.parent.insertBefore(f[e],k);this.cloneNodes.splice(g+1,0,{data:d[b],cloneData:JSON.parse(JSON.stringify(d[b])),nodes:f})}return this};a.prototype.bindedNodes=function(a){var c=document.createElement("div");
c.innerHTML=this.elemHTML;if(a)for(var b=Object.keys(a),e=0;e<b.length;e++)c.innerHTML=c.innerHTML.replace(new RegExp("{{"+b[e]+"}}","g"),a[b[e]]);return Array.prototype.slice.call(c.childNodes)};return a}();f.Element=h})(DynamicLoad||(DynamicLoad={}));
