var DynamicLoad;
(function(f){var h=function(){function a(c,b){this.cloneNodes=[];if(c instanceof HTMLElement){if(componentHandler){var a=c.querySelectorAll("[data-upgraded]");0<a.length&&componentHandler.downgradeElements(a)}a=document.createElement("div");a.appendChild(c.cloneNode(!0));this.element=c;this.elemHTML=a.innerHTML;c.style.display="none";b&&(b.innerHTML="");this.parent=b||c.parentNode}else this.elemHTML=c,b&&(b.innerHTML=""),this.parent=b}a.prototype.bind=function(c){for(var b=1;b<this.cloneNodes.length;b++)this.parent.removeChild(this.cloneNodes[b].node);if(0!=
this.cloneNodes.length){this.cloneNodes=[this.cloneNodes[0]];for(var b=Object.keys(c),a=0;a<b.length;a++)if(this.cloneNodes[0].cloneData[b[a]]!=c[b[a]]){b=this.bindedNodes(c);for(a=0;a<b.length;a++)this.parent.insertBefore(b[a],this.cloneNodes[0].nodes[this.cloneNodes[0].nodes.length-1].nextSibling);for(a=0;a<this.cloneNodes[0].nodes.length;a++)this.parent.removeChild(this.cloneNodes[0].nodes[a]);this.cloneNodes[0].nodes=b;this.cloneNodes[0].cloneData=JSON.parse(JSON.stringify(c));break}}else{for(var b=
this.bindedNodes(c),e=this.element?this.element.nextSibling:this.parent.firstChild,a=0;a<b.length;a++)this.parent.insertBefore(b[a],e);this.cloneNodes.push({data:c,cloneData:JSON.parse(JSON.stringify(c)),nodes:b})}return this};a.prototype.refresh=function(){for(var c=0;c<this.cloneNodes.length;c++)for(var a=Object.keys(this.cloneNodes[c].data),d=0;d<a.length;d++)if(this.cloneNodes[c].cloneData[a[d]]!=this.cloneNodes[c].data[a[d]]){a=this.bindedNodes(this.cloneNodes[c].data);for(d=0;d<a.length;d++)this.parent.insertBefore(a[d],
this.cloneNodes[c].nodes[this.cloneNodes[c].nodes.length-1].nextSibling);for(d=0;d<this.cloneNodes[c].nodes.length;d++)this.parent.removeChild(this.cloneNodes[c].nodes[d]);this.cloneNodes[c].nodes=a;this.cloneNodes[c].cloneData=JSON.parse(JSON.stringify(this.cloneNodes[c].data));break}return this};a.prototype.repeat=function(c,a){for(var d=0;d<this.cloneNodes.length;d++)if(-1==c.indexOf(this.cloneNodes[d].data)){for(var e=0;e<this.cloneNodes[d].nodes.length;e++)this.parent.removeChild(this.cloneNodes[d].nodes[e]);
this.cloneNodes.splice(d,1);--d}a&&this.refresh();for(var g=this.cloneNodes.length-1,d=c.length-1;0<=d;d--)if(-1!=g&&this.cloneNodes[g].data===c[d])g--;else{for(var f=this.bindedNodes(c[d]),k=-1==g?this.element?this.element.nextSibling:this.parent.firstChild:this.cloneNodes[g].nodes[this.cloneNodes[g].nodes.length-1].nextSibling,e=0;e<f.length;e++)this.parent.insertBefore(f[e],k);this.cloneNodes.splice(g+1,0,{data:c[d],cloneData:JSON.parse(JSON.stringify(c[d])),nodes:f})}return this};a.prototype.bindedNodes=
