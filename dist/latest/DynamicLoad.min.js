var DynamicLoad;
(function(f){var h=function(){function a(c,b){this.cloneNodes=[];if(c instanceof HTMLElement){var a=document.createElement("div");a.appendChild(c.cloneNode(!0));this.element=c;this.elemHTML=a.innerHTML;c.style.display="none";b&&(b.innerHTML="");this.parent=b||c.parentNode}else this.elemHTML=c,b&&(b.innerHTML=""),this.parent=b}a.prototype.bind=function(c){for(var b=1;b<this.cloneNodes.length;b++)this.parent.removeChild(this.cloneNodes[b].node);if(0!=this.cloneNodes.length){this.cloneNodes=[this.cloneNodes[0]];
for(var b=Object.keys(c),a=0;a<b.length;a++)if(this.cloneNodes[0].cloneData[b[a]]!=c[b[a]]){b=this.bindedNodes(c);for(a=b.length-1;0<=a;a--)this.parent.insertBefore(b[a],this.cloneNodes[0].nodes[this.cloneNodes[0].nodes.length-1].nextSibling);for(a=0;a<this.cloneNodes[0].nodes.length;a++)this.parent.removeChild(this.cloneNodes[0].nodes[a]);this.cloneNodes[0].nodes=b;this.cloneNodes[0].cloneData=JSON.parse(JSON.stringify(c));break}}else{for(var b=this.bindedNodes(c),e=this.element?this.element.nextSibling:
this.parent.firstChild,a=b.length-1;0<=a;a--)this.parent.insertBefore(b[a],e);this.cloneNodes.push({data:c,cloneData:JSON.parse(JSON.stringify(c)),nodes:b})}return this};a.prototype.refresh=function(){for(var c=0;c<this.cloneNodes.length;c++)for(var a=Object.keys(this.cloneNodes[c].data),d=0;d<a.length;d++)if(this.cloneNodes[c].cloneData[a[d]]!=this.cloneNodes[c].data[a[d]]){a=this.bindedNodes(this.cloneNodes[c].data);for(d=a.length-1;0<=d;d--)this.parent.insertBefore(a[d],this.cloneNodes[c].nodes[this.cloneNodes[c].nodes.length-
1].nextSibling);for(d=0;d<this.cloneNodes[c].nodes.length;d++)this.parent.removeChild(this.cloneNodes[c].nodes[d]);this.cloneNodes[c].nodes=a;this.cloneNodes[c].cloneData=JSON.parse(JSON.stringify(this.cloneNodes[c].data));break}return this};a.prototype.repeat=function(c,a){for(var d=0;d<this.cloneNodes.length;d++)if(-1==c.indexOf(this.cloneNodes[d].data)){for(var e=0;e<this.cloneNodes[d].nodes.length;e++)this.parent.removeChild(this.cloneNodes[d].nodes[e]);this.cloneNodes.splice(d,1);--d}a&&this.refresh();
for(var g=this.cloneNodes.length-1,d=c.length-1;0<=d;d--)if(-1!=g&&this.cloneNodes[g].data===c[d])g--;else{for(var f=this.bindedNodes(c[d]),k=-1==g?this.element?this.element.nextSibling:this.parent.firstChild:this.cloneNodes[g].nodes[this.cloneNodes[g].nodes.length-1].nextSibling,e=f.length-1;0<=e;e--)this.parent.insertBefore(f[e],k);this.cloneNodes.splice(g+1,0,{data:c[d],cloneData:JSON.parse(JSON.stringify(c[d])),nodes:f})}return this};a.prototype.bindedNodes=function(c){var a=document.createElement("div");
a.innerHTML=this.elemHTML;if(c)for(var d=Object.keys(c),e=0;e<d.length;e++)a.innerHTML=a.innerHTML.replace(new RegExp("{{"+d[e]+"}}","g"),c[d[e]]);console.log(a.innerHTML);console.log(a.childNodes.length);for(e=0;e<a.childNodes.length;e++)console.log(a.childNodes[e]);c=[];c.length=a.childNodes.length;for(e=0;e<a.childNodes.length;e++)c[e]=a.childNodes[e];return c};return a}();f.Element=h})(DynamicLoad||(DynamicLoad={}));
(function(f){var h=function(){function a(a,b,d){this.callback=[];this.method=a;this.url=b;this.setData(d)}a.prototype.addCallback=function(a){this.callback.push(a);return this};a.prototype.send=function(a){var b=this,d=this.callback,e=new XMLHttpRequest,g=function(e,g){e&&(b.status=e);g&&(b.responseText=g);var f;f="json"==a?JSON.parse(b.responseText):b.responseText;for(var r=0;r<d.length;r++)d[r](e,f)};if(this.status)g();else{var f=this.url+(-1==this.url.indexOf("?")?"?":"&")+"_="+(new Date).getTime();
...
{}));
(function(f){var h=function(){function a(a){var b=this;this.isAppend=!1;this.callback=[];this.src=a;this.http=f.Http.get(a).addCallback(function(a,c){var g=document.createElement("div");g.innerHTML=c;if(b.dataBindRule)for(var f=Object.keys(b.dataBindRule),k=0;k<f.length;k++)g.innerHTML=g.innerHTML.replace(new RegExp("{{"+f[k]+"}}","g"),b.dataBindRule[f[k]]);b.isAppend||(b.element.innerHTML="");(function(a){for(var c=g.childNodes,b=0;b<c.length;b++)a.appendChild(c[b])})(b.element);componentHandler&&componentHandler.upgradeAllRegistered();
