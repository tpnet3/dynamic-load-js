...
f.open(this.method,d,!0);f.setRequestHeader("Content-Type","application/json");this.auth&&f.setRequestHeader("Authorization",this.auth);f.onload=function(){g(f.status,f.responseText)};this.data?f.send(this.data):f.send()}};a.prototype.setAuth=function(a){this.auth=a;return this};a.prototype.setData=function(a){this.data=a;"string"!=typeof this.data&&(this.data=JSON.stringify(this.data));return this};a.prototype.asString=function(a){a&&this.addCallback(a);this.send("string")};a.prototype.asJson=function(a){a&&
this.addCallback(a);this.send("json")};a.get=function(e){return new a("GET",e)};a.put=function(e,b){return(new a("PUT",e)).setData(b)};a.post=function(e,b){return(new a("POST",e)).setData(b)};a["delete"]=function(e,b){return(new a("DELETE",e)).setData(b)};return a}();d.Http=h})(DynamicLoad||(DynamicLoad={}));
...
(function(d){var h=function(){function a(){}a.uriListener=function(a){d.Location.enchantUrl()&&a();window.addEventListener("hashchange",function(){d.Location.enchantUrl()&&a()})};a.run=function(e){var b=function(c,g,e){var k=function(a){var c={},f=a[g];if(!f)for(var d=Object.keys(a),k=0;k<d.length;k++){var n=new RegExp("^"+d[k].replace(/\{\*}$/,"(.+)").replace(/\{([^\/]+)}/g,"([^\\/]+)")+"$");if(n.test(g)){for(var f=n.exec(d[k]).slice(1),n=n.exec(g).slice(1),h=0;h<f.length;h++)c[f[h].replace(/^\{(.+)}$/,
"$1")]=n[h];f=a[d[k]];break}}"string"==typeof f?b(f,g,e):e(f,c)};a.routes[c]?k(a.routes[c]):d.Http.get(c).addCallback(function(b,d){a.routes[c]=d;k(d)}).asJson()},c=function(a,b,c,d){if(a){a=a.split(".");for(var e=0;e<a.length;e++)if(b[a[e]])b=b[a[e]];else return;"function"==typeof b&&b(d,c)}};a.uriListener(function(){for(var f=d.Location.path(),g=0;g<a.curJs.length;g++)a.curJs[g].options.reload&&(c(a.curJs[g].options.destroy,window,a.curJs[g].options.data,a.curParams),a.curJs[g].loadJs.destroy(),
a.curJs.splice(g,1),g--);var h=[],k=[],v=function(){a.curHtml&&(a.curHtml.destroy(),a.curHtml=null)},p=function(b){for(var d=0;d<a.curJs.length;d++)c(a.curJs[d].options.destroy,window,a.curJs[d].options.data,b),a.curJs[d].loadJs.destroy();a.curJs=h};b(e,f,function(b,e){var f={},g;for(g in a.curParams)a.curParams.hasOwnProperty(g)&&(f[g]=a.curParams[g]);a.curParams=e;if(b){var r=function(c){if(b.html){var e=document.getElementById("body");e||(e=document.getElementsByTagName("body")[0]);a.curHtml=d.LoadHtml.getInstance(b.html);
c&&a.curHtml.addCallback(c);a.curHtml.addCallback(function(){p(f)});a.curHtml.put(e)}else c&&c(),p(f),v()},w=function(e){for(var f=0;f<e.length;f++){var g=b.js[e[f]];h.push({loadJs:d.LoadJs.getInstance(e[f]).addCallback(function(){c(g.create,window,g.data,a.curParams)}).load(),options:g})}};(function(){if(b.css)for(var c=0;c<b.css.length;c++)k.push(d.LoadCss.getInstance(b.css[c]).load());for(c=0;c<a.curCss.length;c++)a.curCss[c].destroy();a.curCss=k})();if(b.js){var l=Object.keys(b.js);g=!1;for(var m=
0,t=0;m<l.length;m++)if(b.js[l[m]].preload){var u=b.js[l[m]];g=!0;t++;h.push({loadJs:d.LoadJs.getInstance(l[m]).addCallback(function(){c(u.create,window,u.data,a.curParams);t--;0==t&&r(function(){w(l)})}).load(),options:u});l.splice(m,1);m--}g||r(function(){w(l)})}else r()}else p(f),v()})})};a.routes={};a.curParams={};a.curCss=[];a.curJs=[];return a}();d.Route=h})(DynamicLoad||(DynamicLoad={}));
